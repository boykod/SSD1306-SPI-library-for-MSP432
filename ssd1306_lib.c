...
//*******************************************************************************************************************************************
void sendData (unsigned char *params, unsigned char flag, unsigned char isData) {
    TI_transmit_field = params;
    spi_transmit(TI_transmit_field, flag, isData);
}
//*******************************************************************************************************************************************
...
//    buf[0] = SSD1306_DATA_MODE; // fist item "send data mode"
    while (str[i] != '\0') {
        ...
        sendData(buf, FONT6X8_WIDTH + 1, 1); // send the buf to display
        x += FONT6X8_WIDTH;
        i++;
  };
}
...
void drawImage(unsigned char x, unsigned char y, unsigned char sx,
    ...
    if (((x + sx) > SSD1306_WIDTH) || ((y + sy) > SSD1306_HEIGHT) || (sx == 0) || (sy == 0)) return;
...
//    dataBuffer[0] = SSD1306_DATA_MODE; // fist item "send data mode"
    for (p=p0; p<(p1+1); p++) {
        ...
        sendData(dataBuffer, sx + 1, 1); // send the buf to display
    }
    free(dataBuffer);
}
//******************************************************************************************************************************************
